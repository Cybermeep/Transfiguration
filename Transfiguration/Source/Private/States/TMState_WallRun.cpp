...
bool TMState_WallRun::DetectWall(FVector& OutNormal, bool& OutOnLeft) const
{
    ...
    FVector RightEnd = Start + Owner->GetActorRightVector() * TraceLen;
    ...
    FVector LeftEnd = Start - Owner->GetActorRightVector() * TraceLen;
...
bool TMState_WallRun::CanEnter(ATMCharacter* Char)
{
    ...
    FVector Dummy;
    bool bDummy;
    TMState_WallRun Tester(Char);

    if (!Tester.DetectWall(Dummy, bDummy)) return false;

    if (!Char->CanWallRunAtLocation(Char->GetActorLocation()))
    {
        return false;
    }
...
void TMState_WallRun::Enter()
{
    ...
    DetectWall(WallNormal, bWallOnLeft);
